  ...
  <div class="container-equipment-inspection">
    <form class="form-horizontal" id="createForm" method="post" accept-charset="utf-8" enctype="multipart/form-data">
      <div class="card-body table-responsive">
        <div class="row">
          <div class="col-12">
                ...
                <table id="table-equipment-inspection-11" class="table table-bordered m-0" style="width: 100%;">
                  <thead id="head-equipment-inspection-11">
                    <tr class="text-thin text-center">
                      <th style="vertical-align: middle;" colspan="2"><div class="text-bold">{{ dailyDate }}</div></th>
                    </tr>
                    <tr class="text-thin text-center">
                      <th style="vertical-align: middle;" colspan="2"><div class="text-bold">昼勤 Day</div></th>
                    </tr>
                  </thead>
                </table>
              </div>
              <div class="col-9" style="padding-left: 0px;">
                <table id="table-equipment-inspection-12" class="table table-bordered m-0" style="width: 100%;">
                  <thead id="head-equipment-inspection-12">
                    <tr class="text-thin text-center">
                      <th colspan="2" style="width: 4rem; vertical-align: middle;"><div class="text-bold">日々生産管理表</div></th>
                      <th class="bg-light" style="width: 4rem; vertical-align: middle;">
                        <div style="width: 4rem;">ラインNo.</div>
                      </th>
                      <th style="width: 4rem; vertical-align: middle;" class="bg-td-green text-center" @click="showModalLine($event)">
                        <div style="width: 4rem;" class="form-group m-0">
                          ...
                          <input id="lineCODE" type="hidden" name="" :value="line_code" class="form-control bg-td-green">
                      ...
                      <th style="width: 4rem; vertical-align: middle;" colspan="2" class="bg-td-green" @click="showModalProduct($event)">
                        <div class="form-group m-0">
                          <input id="productID" type="text" name="product_id" :value="product_id" class="form-control bg-td-green">
                      ...
                      <th style="vertical-align: middle;" colspan="2" class="bg-td-white">
                        <div style="width:;" class="form-group m-0">
                          <!-- <input id="addressID" type="text" name="address_id" class="form-control bg-td-white"> -->
                          <p v-if="product_id" style="margin-bottom: 0px;">{{ `${product_department_name}` }}</p>
                    ...
                    <tr class="text-thin text-center">
                      <th colspan="2" style="width: 7.2rem;">
                        <div style="width: 100%;">
                          <button style="font-size: 14px;" @click="changeModeComment" type="button" class="btn btn-primary bg-search">備考入力モード</button>
                        </div>
                      </th>
                      <th style="width: 4rem; vertical-align: middle;" class="bg-light">
                        <div>C/T</div>
                      ...
                      <th style="width: 4rem; vertical-align: middle;" colspan="2" class="bg-td-white">
                        <div class="form-group text-center m-0">
                          <input @blur="changeCtInput($event)" id="ctID" type="number" min="0" v-model="dailyDataTable.ct_id" name="ct_id" max="100" maxlength="6" class="form-control bg-td-green">
                        </div>
                      </th>
                      <th colspan="8">
                        <div class="form-group row m-0">
                          <div class="col-sm-1 col-form-label pr-0">100%:</div>
                          <div class="col-sm-1 col-form-label col-form-value pl-0"><span v-if="dailyDataTable.ct_id">{{ Math.round(3600 / parseInt(dailyDataTable.ct_id) * 1 * 100) / 100 }}</span></div>
                          <div class="col-sm-1 col-form-label pr-0">90%:</div>
                          <div class="col-sm-1 col-form-label col-form-value pl-0"><span v-if="dailyDataTable.ct_id">{{ Math.round(3600 / parseInt(dailyDataTable.ct_id) * 0.9 * 100) / 100 }}</span></div>
                          <div class="col-sm-1 col-form-label pr-0">85%:</div>
                          <div class="col-sm-1 col-form-label col-form-value pl-0"><span v-if="dailyDataTable.ct_id">{{ Math.round(3600 / parseInt(dailyDataTable.ct_id) * 0.85 * 100) / 100 }}</span></div>
                          <div class="col-sm-1 col-form-label pr-0">80%:</div>
                          <div class="col-sm-1 col-form-label col-form-value pl-0"><span v-if="dailyDataTable.ct_id">{{ Math.round(3600 / parseInt(dailyDataTable.ct_id) * 0.8 * 100) / 100 }}</span></div>
                          <div class="col-sm-1 col-form-label pr-0">75%:</div>
                          <div class="col-sm-1 col-form-label col-form-value pl-0"><span v-if="dailyDataTable.ct_id">{{ Math.round(3600 / parseInt(dailyDataTable.ct_id) * 0.75 * 100) / 100 }}</span></div>
                        </div>
                      </th>
                      <th v-for="i in (33-15)" style="display: none;"></th>
                    </tr>
              ...
              <div class="col-3" style="padding-right: 0px;">
                <table id="table-equipment-inspection-21" class="table-s0 table table-bordered">
                  ...
                  <tbody>
                    ...
                    <tr v-else>
                      <td colspan="2" class="text-center" :class="row.key === 'man_hours_per_machine' ? 'bg-td-yellow' : (row.edit ? 'bg-td-white' : 'bg-computation')"><div>{{ row.name}}</div></td>
                    </tr>
                    </template>
                  </tbody>
                </table>
                  ...
                  <div class="table-s2">
                    <table id="table-equipment-inspection-22" class="table-s0 table-s3 table table-bordered">
                      ...
                      <tbody>
                        ...
                        <template v-if="row.data && row.data.length">
                        <tr>
                          <td @click="showModalComment($event, row, row.data[0], i-1)" v-for="i in 31" :class="row.data[0].edit ? ((mode_comment && row.key !== 'supervisor_confirmation') ? 'bg-td-comment' : 'bg-td-green-important') : 'bg-computation'"><p style="margin-bottom: 0px;">
                            <div :class="mode_comment ? 'mode-comment' : 'not-mode-comment'" v-if="(dailyDataTable[row.key] && dailyDataTable[row.key][row.data[0].key] && dailyDataTable[row.key][row.data[0].key][i-1] && dailyDataTable[row.key][row.data[0].key][i-1].comment)"><i class="far fa-comment-dots"></i></div>
                            <div v-if="(!mode_comment && row.data[0].edit && dailyDataTable[row.key] && dailyDataTable[row.key][row.data[0].key] && dailyDataTable[row.key][row.data[0].key][i-1] && dailyDataTable[row.key][row.data[0].key][i-1])" class="form-group m-0"><input style="text-align: center;" :value="dailyDataTable[row.key][row.data[0].key][i-1].num_input" v-on:keyup="changeNumInput($event, row, row.data[0], i-1)" type="number" min="1" class="form-control bd-none bg-td-green"></div>
                            <div style="text-align: center;" v-if="['number_of_prod'].includes(row.data[0].key)">{{ getAchievementPercent(i-1, row.data[0].key) }}</div>

                          </p></td>
                        </tr>
                        <tr v-for="j in (row.data.length - 1)" :id="`row_${row.key}${j}`">
                          <td @click="showModalComment($event, row, row.data[j], i-1)" v-for="i in 31" :class="row.data[j].edit ? ((mode_comment && row.key !== 'supervisor_confirmation') ? 'bg-td-comment' : 'bg-td-green-important') : 'bg-computation'"><p style="margin-bottom: 0px;">
                            <div :class="mode_comment ? 'mode-comment' : 'not-mode-comment'" v-if="(dailyDataTable[row.key] && dailyDataTable[row.key][row.data[j].key] && dailyDataTable[row.key][row.data[j].key][i-1] && dailyDataTable[row.key][row.data[j].key][i-1].comment)"><i class="far fa-comment-dots"></i></div>
                            <div v-if="(!mode_comment && row.data[j].edit && dailyDataTable[row.key] && dailyDataTable[row.key][row.data[j].key] && dailyDataTable[row.key][row.data[j].key][i-1] && dailyDataTable[row.key][row.data[j].key][i-1])" class="form-group m-0"><input style="text-align: center;" :value="dailyDataTable[row.key][row.data[j].key][i-1].num_input" v-on:keyup="changeNumInput($event, row, row.data[j], i-1)" type="number" min="1" class="form-control bd-none bg-td-green"></div>
                        ...
                        <tr v-else class="colspan-2">
                          <td @click="showModalComment($event, null, row, i-1)" v-for="i in 31" :class="row.key === 'man_hours_per_machine' ? 'bg-td-yellow' : (row.edit ? ((mode_comment && row.key !== 'remarks' && row.key !== 'supervisor_confirmation') ? 'bg-td-comment' : 'bg-td-green-important') : 'bg-computation')"><p style="margin-bottom: 0px;">
                            <div :class="mode_comment ? 'mode-comment' : 'not-mode-comment'" v-if="(dailyDataTable[row.key] && dailyDataTable[row.key][i-1] && dailyDataTable[row.key][i-1].comment)"><i class="far fa-comment-dots"></i></div>
                            <div v-if="(!mode_comment && (row.key !== 'remarks' && row.key !== 'supervisor_confirmation') && row.edit && dailyDataTable[row.key] && dailyDataTable[row.key][i-1])" class="form-group m-0"><input style="text-align: center;" :value="dailyDataTable[row.key][i-1].num_input" v-on:keyup="changeNumInput($event, null, row, i-1)" type="number" min="1" class="form-control bd-none bg-td-green"></div>
                            ...
                            <div style="text-align: center;" v-if="row.key === 'supervisor_confirmation' && dailyDataTable[row.key] && dailyDataTable[row.key][i-1]">{{ dailyDataTable[row.key][i-1].confirmed_name }}</div>
                          </p></td>
                        </tr>
                        </template>
                      </tbody>
            ...
            <div class="row">
              <div class="col-3" style="padding-right: 0px;">
                <table v-if="line_id && dailyDataTable.machines && dailyDataTable.machines.length" id="table-equipment-inspection-31" class="table-s0 table table-bordered mt-3" :set="totalUsed = getTotalUsed()">
              ...
              <div class="col-9" style="padding-left: 0px;">
                <div class="table-s1">
                  <div class="table-container" style="max-width: 100% !important; overflow-x: auto; text-align:center;">
                    <table v-if="line_id && dailyDataTable.machines && dailyDataTable.machines.length" 
                      style="width: 100%"
                      id="table-equipment-inspection-32" class="table-s3 table table-bordered mt-3" 
                      :set="totalUsed = getTotalUsed()">
                      <thead id="head-machine-32" >
                        <tr class="text-thin text-center">
                          <template v-if="dailyDataTable.machines && dailyDataTable.machines.length">
                          <th style=" max-height: 30px; overflow-y: auto; padding-top: 0; padding-bottom: 0;" class="bg-light" v-for="(machine, index) in dailyDataTable.machines">{{(machine.machine_name) ? `${machine.machine_number}: ${machine.machine_name}` :  `機番 ${machine.machine_number}` }}</th>
                          </template>
                        </tr>
                      </thead>
                      <tbody id="body-machine-32">
                        <tr class="tr-row-all text-center">
                          <template v-if="dailyDataTable.machines && dailyDataTable.machines.length">
                          <td style=" max-height: 30px; overflow-y: auto; padding-top: 0; padding-bottom: 0;" :class="getTotalMachineUsed(machine, 'el')" v-for="(machine, index) in dailyDataTable.machines">{{ getTotalMachineUsed(machine, 'number') }}</td>
                          </template>
                        </tr>
                        <tr class="tr-row text-center" v-if="max_total">
                          <template v-for="(machine, index) in dailyDataTable.machines">
                            <td style=" max-height: 30px; overflow-y: auto; padding-top: 0; padding-bottom: 0;" v-if="machine.json_data && machine.json_data[0]" @click="showModalResetCutlery($event, machine.json_data[0], index, 0)" :class="getTotalMachineUsed(machine.json_data[0], 'c_el')">
                              <div v-if="machine.json_data && machine.json_data[0]" style="width: 200px; overflow-y: auto; ">
                        ...
                        <tr v-if="max_total" class="tr-row text-center" v-for="i in (max_total - 1)">
                          <template v-for="(machine, index) in dailyDataTable.machines">
                              <td style=" max-height: 30px; overflow-y: auto; padding-top: 0; padding-bottom: 0;" v-if="machine.json_data && machine.json_data[i]" @click="showModalResetCutlery($event, machine.json_data[i], index, i)" :class="getTotalMachineUsed(machine.json_data[i], 'c_el')">
                                <div v-if="machine.json_data && machine.json_data[i]" style="width: 200px;">
      ...
      <div class="card-footer">
        <button v-if="false" type="button" class="btn btn-success mr-1" id="modal-btn-submit" @click="dailyReference()">日々生産管理表 参照</button>
      </div>
    </form>
  </div>
</template>
...
export default {
  ...
  setup() {
    ...
    const product_department_name = ref('');
    ...
    const showModalDepartment = (event) => {
      ...
      $('#basic-set').hide();
      $('#comment-set').hide();
      $('#reset-cutlery-set').hide();
      $('#modal-xl-create').modal('show');
    };

    const showModalResetCutlery = (event, _item, index, i) => {
      var c_el = getTotalMachineUsed(_item, 'c_el');
      $('#reset-cutlery-set').show();
      $('#reset-cutlery-set').attr('data-index', '');
      $('#reset-cutlery-set').attr('data-i', '');
      $('#department-set').hide();
      $('#product-set').hide();
      $('#line-set').hide();
      $('#basic-set').hide();
      ...
      $('#reset-cutlery-set').attr('data-index', index);
      $('#reset-cutlery-set').attr('data-i', i);
      $('#modal-xl-create').modal('show');
    };
    ...
    const showModalComment = (event, rowP, row, index) => {
      ...
      if (row.key === 'supervisor_confirmation') {
        if (!(_dailyDataTable[row.key] && _dailyDataTable[row.key][index])) {
          return;
        } else {
          var confirmed_name = _dailyDataTable[row.key][index].confirmed_name ? '' : user_name;
          btnLineCode({confirmed_name, index}, 'supervisor_confirmation');
        }
        return
      }
      $('#comment-set').hide();
      ...
      $('#department-set').hide();
      $('#basic-set').hide();
      $('#exampleComment1').attr('data-p-code', '');
      $('#exampleComment1').attr('data-code', '');
      $('#exampleComment1').attr('data-index', '');
      if (row.key === 'remarks') {
        if (!(_dailyDataTable[row.key] && _dailyDataTable[row.key][index])) {
          return;
        } else {
        ...
        $('#exampleComment1').val(comment);
        $('#exampleComment1').attr('data-code', row.key);
        $('#exampleComment1').attr('data-index', index);
        $('#comment-set').show();
        $('#modal-xl-create').modal('show');
        return;
      }
      if (!mode_comment.value) {
        return;
      }
      if (rowP && rowP.key) {
        ...
        $('#exampleComment1').attr('data-p-code', rowP.key);
      } else {
        if (!(_dailyDataTable[row.key] && _dailyDataTable[row.key][index])) {
          return;
        } else {
    ...
    const changeCtInput = (event) => {
      ...
      if (inputVal != oldCtInput) {
        ...
        var _dailyDataTable = dailyDataTable.value;
        var arr_ct_input = _dailyDataTable.arr_ct_input ? _dailyDataTable.arr_ct_input : null;
        var supervisor_confirmation = _dailyDataTable.supervisor_confirmation ? _dailyDataTable.supervisor_confirmation : null;

        console.log(confirmedNameIndex)
        console.log(_dailyDataTable)
        console.log(arr_ct_input)
        console.log(supervisor_confirmation)
        ...
        if (arr_ct_input && arr_ct_input.length) {
            for (var i = 0; i < arr_ct_input.length; i++) {
                // if (arr_ct_input[i].index > confirmedNameIndex) {
                    arr_ct_input[i].ct_input = inputVal;
                // }
            }
        }

        dailyDataTable.value.arr_ct_input = arr_ct_input;
        submitDailyForm();
        oldCtInput = inputVal;
      }
    };
    const changeNumInput = (event, rowP, row, index) => {
      ...
      if (rowP && rowP.key && dailyDataTable.value[rowP.key]
        ...
        if (oldId != inputVal) {
          setTimeout(() => {
            // update machine total used
            if (['finished_prod', 'no_materials', 'judgment_prod'].includes(row.key)) {
              var _machines = dailyDataTable.value.machines;
              if (_machines && _machines.length) {
                for (var i = 0; i < _machines.length; i++) {
                  if (_machines[i].reset) {
                    _machines[i].total_used = _machines[i].total_used ? parseInt(_machines[i].total_used) + parseInt(inputVal) : parseInt(inputVal);
                  } else {
                    _machines[i].total_used = total_used.value;
                  }
                  if(_machines[i].json_data && _machines[i].json_data.length) {
                    for (var j = 0; j < _machines[i].json_data.length; j++) {
                      if (_machines[i].json_data[j].reset) {
                        _machines[i].json_data[j].total_used = _machines[i].json_data[j].total_used ? parseInt(_machines[i].json_data[j].total_used) + parseInt(inputVal) : parseInt(inputVal);
                      } else {
                        _machines[i].json_data[j].total_used = total_used.value;
                      }
                    }
                  }
                }
              ...
              // console.log(dailyDataTable.value.machines);
            }
            // end
            submitDailyForm();
          }, 3000);
    ...
    const checkCtLastConfirmDay = (ctInput) => {
      ...
      // console.log(ctInput.index, confirmedNameIndex)
      // if (parseInt(ctInput.index) > confirmedNameIndex) {
      //   return 'bg-td-white';
      // }
      // question here, although walang pinagkaiba ang color kapag may confirmed name at wala, bakit nakacondition
      // ito ng pagbabago ng kulay
    ...
    const btnLineCode = (data, type) => {
      ...
      if (type === 'lines') {
        ...
        if (id) {
          ...
          product_department_name.value = data.department_name;
        } else {
          product_id.value = '';
          product_name.value = '';
          product_code.value = '';
      ...
      } else if (type === 'supervisor_confirmation') {
        if (dailyDataTable.value['supervisor_confirmation'] && dailyDataTable.value['supervisor_confirmation'][data.index]) {
          dailyDataTable.value['supervisor_confirmation'][data.index].confirmed_name = data.confirmed_name;
          submitDailyForm();
        }
      } else if (type === 'resetCutlery') {
        if (dailyDataTable.value.machines && dailyDataTable.value.machines[data.index] && dailyDataTable.value.machines[data.index].json_data && dailyDataTable.value.machines[data.index].json_data[data.i]) {
          dailyDataTable.value.machines[data.index].json_data[data.i].reset = true;
          dailyDataTable.value.machines[data.index].json_data[data.i].total_used = 0;
          submitDailyForm();
        }
      }
    ...
    const submitDailyForm = (_type) => {
      ...
      var data = {
        ...
        product_code: product_code.value, product_model: product_model.value,
        product_short_name: product_short_name.value, product_department_name: product_department_name.value,
    ...
    const initialDataTable = (initial) => {
      ...
      if(initial.id) {
        daily_production_control_id = initial.id;
      }
      ...
      product_department_name.value = initial.json_data.product_department_name ?? '';
      var _machines = [];
    ...
    const getCtInput = (item) => {
      ...
      return item.ct_input ? item.ct_input : dailyDataTable.value.ct_id;
    ...
    const getTotalUsed = () => {
      ...
      if (number_of_prod && number_of_prod.length) {
        for (var i = 0; i < number_of_prod.length; i++) {
          total += number_of_prod[i].num_input ? parseFloat(number_of_prod[i].num_input) : 0;
        }
      }
    ...
    const dailyReference = () => {
      return redirectToUrl(link +'/' + daily_production_control_id +'/reference');
    };
    ...
    return {
      ...
      basic_set_id, items,
      showModalUpload, showModalDepartment, showModalProduct, showModalLine, initialItem,
      ...
      product_id, product_name, product_code, product_model, product_short_name, product_department_name,
      // ct_id, achievement, deadtime, actual_time_hours, man_hours_per_machine,
      // remarks, supervisor_confirmation, total_time_hours, machines, arr_ct_input,
      ...
      getAchievementDeadTimePercent, dailyReference, checkCtLastConfirmDay,
      showModalResetCutlery, getTotalMachineUsed,
    };
  },
  mounted() {
