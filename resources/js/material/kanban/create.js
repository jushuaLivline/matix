$(function () {
  ...
  barcodeInput.addEventListener("keydown", function (event) {
    if (event.key === "Enter") {
      ...
      try {
        $.ajax({
          url: "/material/ajax-check-kanban-management-no",
          type: "GET",
  ...
  function confirmDelete(button) {
    if (
      ...
      const supplyMaterialOrderId = button.getAttribute("data-management-no");
      ...
      fetch(`/material/kanban/${supplyMaterialOrderId}`, {
        method: "PUT",
        headers: {
        ...
        .catch((error) => {
          // Handle the error
          alert("供給材料注文の削除中にエラーが発生しました: " + error);
        });
    }
  }
