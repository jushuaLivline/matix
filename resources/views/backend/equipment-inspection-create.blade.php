...
<div class="row">
  <div class="col-12">
    <div class="card">
      ...
      <div data-href="{{ $link }}" id="app-equipment-inspection" data-current-url="{{ url()->current()  }}" data-user-id="{{ Auth::user()->id }}"></div>
      <!-- /.card-body -->
    </div>
    <!-- /.card -->
  </div>
              ...
              <div class="row" id="basic-set" style="display: none;">
                <div class="col-sm-12">
                  <div class="form-group mb-3">
                    ...
                    <input name="process_id" type="number" min="1" max="99" class="form-control text-right col-sm-3" id="exampleProcess1">
                  </div>
                  <div class="form-group">
                    <label class="form-label dotted indented" for="exampleBasicSet1">点検項目基本セット選択</label>
                    <select name="_basic_set_id" id="exampleBasicSet1" class="form-control select2" style="width: 100%;">
                      @if (isset($inspectionItem))
                      <option value="" style="background-color: gray;">点検項目基本セット</option>
                      @foreach ($inspectionItem as $key => $value)
                      <option value="{{ $value->id }}">{{ $value->inspection_item_set }}</option>
                      @endforeach
                      @endif
                    </select>
                  </div>
                  <div class="form-group mb-0" style="margin-top: 1rem;">
                    <button onclick="btnProcess(this)" type="button" class="col-sm-2 btn btn-primary bg-search">選択</button>
                  </div>
                </div>
              </div>
...
<!-- /.modal -->
<div class="modal fade" id="modal-xl-upload" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog modal-xl w_percent_30 modal-dialog-scrollable">
    <div class="modal-content">
          ...
          <div class="card-body">
            ...
            <div class="col-12">
              <div class="form-group row">
                <label for="exampleInputFile" class="col-sm-3 col-form-label btn btn-default bg-tr-gray">参照...</label>
                <div class="col-sm-9">
                  <input onchange="uploadFile(this)" ref="file" style="visibility:hidden; padding: 0px; height: 0px;" accept="image/*" class="form-control" name="logo" type="file" id="exampleInputFile">
                  <input id="exampleInputFileName" style="background-color: transparent;" type="text" class="form-control" disabled>
                </div>
              </div>
            </div>
          </div>
          <div class="card-footer">
              <button onclick="preview(this)" type="button" class="col-sm-3 btn btn-primary bg-search">選択</button>
          </div>
        </div>
      </div>
    </div>
...
@vite('resources/js/tablet/equipment-inspection-create.js')
<script>
  var validator;
  var validatorCreate;
  var dataItems = [];
  ...
  function getMessage(message) {
    ...
    if (message === 'Created equipment inspection success') {
      mes = "作成された設備検査の成功"; //'System error, please try again later!';
    }
    if (message === 'Created equipment inspection failed') {
      mes = "作成された機器の検査に失敗しました"; //'System error, please try again later!';
    }
    if (message === 'required') {
